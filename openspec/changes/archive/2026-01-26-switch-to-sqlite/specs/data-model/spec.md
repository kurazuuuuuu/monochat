...
### Requirement: PostgreSQL UUID Extension
**理由**: SQLiteにはPostgreSQL固有のUUID拡張機能（uuid-ossp、pgcrypto）が存在しないため削除

**Migration**: UUID生成はRuby/RailsのSecureRandomを使用してアプリケーション層で実行
